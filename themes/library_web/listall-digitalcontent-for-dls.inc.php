...
if(!$_ARCHON->Error)
   {
	...
	$listAllCountStart = $_REQUEST['start'];
	$listAllCountEnd = $_REQUEST['end'];
	$listAllType = "";
	if(strlen($listAllCharStart) == 1 && ctype_alpha($listAllCharStart) && strlen($listAllCharEnd) == 1 && ctype_alpha($listAllCharEnd)){
		...
		$listAllType ="char";
	}elseif(ctype_digit($listAllCountStart) && ctype_digit($listAllCountEnd) && $listAllCountStart <= $listAllCountEnd)
	{
		$startingMessage = "Start Number {$listAllCountStart}; End Number {$listAllCountEnd}";
		$listAllType = "count";
	}else{
	...
	} elseif ($listAllType == 'count'){
		$arrDigitalContentAll = $_ARCHON->getAllDigitalContent();        
        $arrDigitalContent = (array_slice($arrDigitalContentAll,$listAllCountStart-1,$listAllCountEnd-$listAllCountStart+1,true));
	}
	//var_dump($arrDigitalContent);
