...
if($_REQUEST['csv']){
	if($_REQUEST['output']){
		...
		$filename = 'atom-csv-digitalcontent-'.strtolower($unitSourcePrefix);
         ...
         foreach($arrCollections as $objCollection)
         {
			...
			if(!empty($objCollection->DigitalContent)){
				...
				foreach($objCollection->DigitalContent as $objDigitalContent){
					...
					//add a unique element if this is the same as the collection identifier
					$parentCollectionIdentifier = $objCollection->Classification->toString(LINK_NONE, true, false, true, false) . '/'. $objCollection->getString('CollectionIdentifier');
					//commenting out because as of 2024-03-25 this is not working for the AtoM import to realize it shouldn't match to the collection and try to update it
					/*
					if($identifierForAtoM == $parentCollectionIdentifier){
						$identifierForAtoM .= "(d)";
					}
					*/
					...
					//add a unique element if this is the same as the collection title
					$parentCollectionTitle = $objCollection->getString('Title');
					if($titleForAtoM == $parentCollectionTitle){
						$titleForAtoM .= " (Digital)";
					}
